...
import { Settings, Brain, Check, AlertCircle, ChevronDown, BookOpen, Trash2, Plus, Search, Clock } from 'lucide-react';
...
export function TopHeader({ settings, books, currentBookId, onModelChange, onOpenSettings, onSelectBook, onDeleteBook, onNewBook }: TopHeaderProps) {
  ...
  return (
    <>
      ...
      <header className="relative flex-shrink-0 w-full h-16 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 flex items-center px-6 md:px-8 z-30">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {modelDropdownOpen && (
              <div className="absolute top-full right-0 mt-2 w-80 bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 max-h-[32rem] overflow-y-auto animate-fade-in-up">
          ...
          <div className="relative">
            ...
            {libraryOpen && (
              <div className="absolute top-full right-0 mt-2 w-80 bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 animate-fade-in-up">
                <div className="p-4 border-b border-white/5">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search library..." className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-white/20 transition-colors text-white placeholder-gray-500" />
                  </div>
                </div>
